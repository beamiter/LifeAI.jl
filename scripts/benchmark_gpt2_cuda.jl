...
2 <= length(ARGS) <= 3 || error(
    "usage: julia --project=. scripts/benchmark_gpt2_cuda.jl " *
...
CUDA.synchronize()
transfer_seconds = elapsed_seconds(transfer_start)
setup = (;
...
for prompt_length in prompt_lengths
    ...
    correctness.passed ||
        error("CUDA cache correctness failed for prompt length $prompt_length")
...
report = (;
    ...
    synchronization=(
        ...
        "the shared benchmark harness; CUDA synchronization is therefore inside " *
