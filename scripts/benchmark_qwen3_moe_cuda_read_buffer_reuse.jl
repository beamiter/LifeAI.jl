...
Threads.nthreads() >= 8 || error(
    "read-buffer-reuse benchmark requires at least eight Julia threads",
)
...
repetitions = parse(
    Int,
    get(ENV, "LIFEAI_MOE_READ_BUFFER_REPETITIONS", "3"),
)
repetitions > 0 || error(
    "LIFEAI_MOE_READ_BUFFER_REPETITIONS must be positive",
)
...
comparisons = (;
    ...
    resident_pool_bytes=first(runs[:pooled]).read_buffer_bytes,
)
...
report = (;
    ...
    methodology=(;
        ...
        mechanism_check=
            "eliminated allocation should equal one raw BF16 payload",
    ),
    configurations,
    comparisons,
    correctness=(; all_outputs_exact=all_outputs_exact[]),
    decision=(;
        enable_read_buffer_reuse_by_default=
            all_outputs_exact[] &&
            comparisons.cold_allocation_reduction > 0.45 &&
            comparisons.revisit_allocation_reduction > 0.45 &&
