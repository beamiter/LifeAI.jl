...
**阶段判断：Qwen3 dense family 真实权重 parity 与 BF16 GPU 推理全闭环，Qwen3-Embedding-0.6B 与最小 dense exact semantic memory 也已完成真实 BF16 parity；RTX 4090 D 上的 dense 容量上限仍是已实证生成的 14B mixed RTN，日常部署选择 Qwen3-8B BF16。原始 Qwen3-30B-A3B 现已具备 40K-capacity BF16 GPU resident/offload session、global/layer-balanced device expert cache，以及直接消费分散 BF16 cache matrices 的 CUDA pointer-table dispatch。**

Chapter 01—28 均已 Closed。[`Chapter 28 — Qwen3 MoE scattered expert cache dispatch`](notes/episodes/episode06_qwen3_moe_and_model_expansion/chapter28_qwen3_moe_scattered_cache.md) 让 scalar CUDA kernel 通过小型 device pointer table 直接读取缓存中的 BF16 expert matrices。真实 30B-A3B 纯命中短请求不再 materialize `10.551 GB` active tensors；推荐每 8 层 GC 的配置从 `12.477 s` 降到 `0.151 s`（`82.89×`），输出逐位一致。完全关闭 forced GC 虽更快，但 100 次重复损失 `0.9375 GiB` allocator free，因此不推荐用于长期服务。
...
- Qwen3 MoE 官方 30B-A3B checkpoint 的 40K cache 容量、短/32-token GPU offload、跨请求 device LRU、中英文自然文本 cache-budget sweep 与 scalar CUDA scattered cache hit 已实跑，但 full-window 40K prefill、长序列生成质量、grouped scattered dispatch、pointer/workspace reuse、异步预取/pinned-memory 双缓冲、完整 AWQ/GPTQ、
