...
**阶段判断：Qwen3 dense family 真实权重 parity 与 BF16 GPU 推理全闭环，Qwen3-Embedding-0.6B 与最小 dense exact semantic memory 也已完成真实 BF16 parity；RTX 4090 D 上的 dense 容量上限仍是已实证生成的 14B mixed RTN，日常部署选择 Qwen3-8B BF16。原始 Qwen3-30B-A3B 现已具备 40K-capacity BF16 GPU resident/offload session，以及 global/layer-balanced 两种 byte-budgeted device expert cache；自然文本顺序扫描的 cache thrashing 已实测并缓解。**

Chapter 01—27 均已 Closed。[`Chapter 27 — Qwen3 MoE layer-balanced expert cache`](notes/episodes/episode06_qwen3_moe_and_model_expansion/chapter27_qwen3_moe_layer_balanced_cache.md) 针对 48 层顺序扫描引起的全局 LRU thrashing，引入按层公平分配 slot 的缓存策略。真实 `English32 → Chinese32 → English32` trace 上，同为 8 GiB 时命中率从 `4.73%` 提升到 `22.56%`，少读/少传 `18.71%`，端到端加速 `1.082×`；综合显存余量后，该 trace 推荐 4 GiB layer-balanced。
...
- Qwen3 MoE 官方 30B-A3B checkpoint 的 40K cache 容量、短/32-token GPU offload、跨请求 device LRU 与中英文自然文本 cache-budget sweep 已实跑，但 full-window 40K prefill、长序列生成质量、device active-tensor concat/GC 优化、异步预取/pinned-memory 双缓冲、完整 AWQ/GPTQ、
