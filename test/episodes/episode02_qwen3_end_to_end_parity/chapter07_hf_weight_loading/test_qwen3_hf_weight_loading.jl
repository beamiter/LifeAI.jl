...
function _qwen3_weight_loading_tensor_bytes(array, dtype::String)
    values = _qwen3_weight_loading_row_major_values(array)
    if dtype == "F32"
...
function _qwen3_weight_loading_write_safetensors(path, specs)
    header = Dict{String,Any}()
    data = UInt8[]
    offset = 0
    for spec in specs
        bytes = _qwen3_weight_loading_tensor_bytes(spec.array, spec.dtype)
...
function _qwen3_weight_loading_write_raw_safetensors(path, header, data::Vector{UInt8})
    header_text = JSON3.write(header)
    padding = mod(-ncodeunits(header_text), 8)
    padded_header = header_text * repeat(" ", padding)
    open(path, "w") do io
...
function _qwen3_weight_loading_config(; kwargs...)
    return merge(
        Dict{String,Any}(
...
function _qwen3_weight_loading_write_config(path; kwargs...)
    write(path, JSON3.write(_qwen3_weight_loading_config(; kwargs...)))
    return path
end

function _qwen3_weight_loading_values(shape; norm=false, seed=0)
    count = prod(shape)
    values = Float32[
...
function _qwen3_weight_loading_qwen_tensors(model::GPTModel)
    tensors = Dict{String,Any}()
    tensors["model.embed_tokens.weight"] = _qwen3_weight_loading_values(
    ...
    for layer in 0:(model.num_layers - 1)
        ...
        tensors["$prefix.input_layernorm.weight"] = _qwen3_weight_loading_values(
            (model.d_model,);
            norm=true,
            seed=10 + layer,
        )
        tensors["$prefix.self_attn.q_proj.weight"] = _qwen3_weight_loading_values(
            (q_dim, model.d_model);
            seed=20 + layer,
        )
        tensors["$prefix.self_attn.k_proj.weight"] = _qwen3_weight_loading_values(
            (kv_dim, model.d_model);
            seed=30 + layer,
        )
        tensors["$prefix.self_attn.v_proj.weight"] = _qwen3_weight_loading_values(
            (kv_dim, model.d_model);
            seed=40 + layer,
        )
        tensors["$prefix.self_attn.o_proj.weight"] = _qwen3_weight_loading_values(
            (model.d_model, q_dim);
            seed=50 + layer,
        )
        tensors["$prefix.self_attn.q_norm.weight"] = _qwen3_weight_loading_values(
            (model.head_dim,);
            norm=true,
            seed=60 + layer,
        )
        tensors["$prefix.self_attn.k_norm.weight"] = _qwen3_weight_loading_values(
            (model.head_dim,);
            norm=true,
            seed=70 + layer,
        )
        tensors["$prefix.post_attention_layernorm.weight"] = _qwen3_weight_loading_values(
            (model.d_model,);
            norm=true,
            seed=80 + layer,
        )
        tensors["$prefix.mlp.gate_proj.weight"] = _qwen3_weight_loading_values(
            (model.mlp_hidden_dim, model.d_model);
            seed=90 + layer,
        )
        tensors["$prefix.mlp.up_proj.weight"] = _qwen3_weight_loading_values(
            (model.mlp_hidden_dim, model.d_model);
            seed=100 + layer,
        )
        tensors["$prefix.mlp.down_proj.weight"] = _qwen3_weight_loading_values(
            (model.d_model, model.mlp_hidden_dim);
            seed=110 + layer,
        )
    end
    tensors["model.norm.weight"] = _qwen3_weight_loading_values(
        (model.d_model,);
        norm=true,
        seed=120,
    )
    model.tie_embeddings || (tensors["lm_head.weight"] = _qwen3_weight_loading_values(
        (model.vocab_size, model.d_model);
        seed=130,
    ))
    return tensors
...
function _qwen3_weight_loading_tensor_specs(tensors; dtype="BF16")
    return [
...
@testset "Qwen3 config parsing and validation" begin
    mktempdir() do directory
        path = _qwen3_weight_loading_write_config(joinpath(directory, "config.json"))
...
@testset "strict safetensors BF16/F32 and sharded index" begin
    mktempdir() do directory
        ...
        shard1 = _qwen3_weight_loading_write_safetensors(
            joinpath(directory, "part-1.safetensors"),
            [(; name="a", dtype="F32", array=Float32[1, 2])],
        )
        shard2 = _qwen3_weight_loading_write_safetensors(
        ...
        bad_dtype_path = _qwen3_weight_loading_write_raw_safetensors(
        ...
        bad_offsets_path = _qwen3_weight_loading_write_raw_safetensors(
...
@testset "Qwen3 parameter mapping, trace, cache, and checkpoint" begin
    mktempdir() do directory
        _qwen3_weight_loading_write_config(joinpath(directory, "config.json"))
        config = load_hf_qwen3_config(joinpath(directory, "config.json"); max_seq_len=16)
        model = GPTModel(config)
        tensors = _qwen3_weight_loading_qwen_tensors(model)
        _qwen3_weight_loading_write_safetensors(
            joinpath(directory, "model.safetensors"),
            _qwen3_weight_loading_tensor_specs(tensors),
        )
