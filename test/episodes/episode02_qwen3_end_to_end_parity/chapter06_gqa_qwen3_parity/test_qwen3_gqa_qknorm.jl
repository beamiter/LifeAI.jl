...
function _qwen3_architecture_gqa_model(;
    vocab_size=61,
    num_kv_heads=2,
    use_qk_norm=true,
    kwargs...,
...
@testset "GQA layer construction and parameters" begin
    ...
    # pre-Qwen3 GQA and QK-Norm implementation (verified elementwise identical against the
...
@testset "GQA + QK-Norm cache correctness matrix" begin
    ...
    for config in configurations
        model = _qwen3_architecture_gqa_model(;
...
@testset "GQA reduces parameters and cache memory" begin
    full = _qwen3_architecture_gqa_model(; num_kv_heads=4, use_qk_norm=false)
    grouped = _qwen3_architecture_gqa_model(; num_kv_heads=2, use_qk_norm=false)
...
@testset "Config round-trip and legacy compatibility" begin
    ...
    # Pre-Qwen3 GQA and QK-Norm configs carry none of the new fields and must default to the
...
@testset "GQA training, checkpoint, resume, generation" begin
    ...
    model = _qwen3_architecture_gqa_model(; vocab_size=vocab_size(tokenizer))
    ...
    checkpoint_path = joinpath(checkpoint_dir, "qwen3_architecture_gqa.checkpoint")
    save_checkpoint(
        checkpoint_path,
        model,
        tokenizer,
