...
const _QWEN3_BF16_ASSETS_PATH = joinpath(
    @__DIR__,
    "fixtures",
    "qwen3_bf16_compute",
    "assets.json",
...
const _QWEN3_FAMILY_SPECS_PATH_FOR_QWEN3_BF16 = joinpath(
    @__DIR__,
    "..",
    "..",
    "episode03_model_family_and_large_weights",
    "chapter11_qwen3_dense_family",
    "fixtures",
    "qwen3_dense_family",
    "specs.json",
)

function _qwen3_bf16_row_major_values(array)
    values = Float32.(array)
    ndims(values) <= 1 && return vec(values)
    return vec(permutedims(values, Tuple(reverse(1:ndims(values)))))
end

function _qwen3_bf16_tensor_bytes(array, dtype::String)
    values = _qwen3_bf16_row_major_values(array)
    if dtype == "F32"
...
function _qwen3_bf16_write_safetensors(path, specs)
    header = Dict{String,Any}()
    data = UInt8[]
    offset = 0
    for spec in specs
        bytes = _qwen3_bf16_tensor_bytes(spec.array, spec.dtype)
...
function _qwen3_bf16_config(; kwargs...)
    return merge(
        Dict{String,Any}(
...
function _qwen3_bf16_values(shape; norm=false, seed=0)
    count = prod(shape)
    values = Float32[
...
function _qwen3_bf16_qwen_tensors(model::GPTModel)
    tensors = Dict{String,Any}()
    tensors["model.embed_tokens.weight"] = _qwen3_bf16_values(
    ...
    for layer in 0:(model.num_layers - 1)
        ...
        tensors["$prefix.input_layernorm.weight"] =
            _qwen3_bf16_values((model.d_model,); norm=true, seed=10 + layer)
        tensors["$prefix.self_attn.q_proj.weight"] =
            _qwen3_bf16_values((q_dim, model.d_model); seed=20 + layer)
        tensors["$prefix.self_attn.k_proj.weight"] =
            _qwen3_bf16_values((kv_dim, model.d_model); seed=30 + layer)
        tensors["$prefix.self_attn.v_proj.weight"] =
            _qwen3_bf16_values((kv_dim, model.d_model); seed=40 + layer)
        tensors["$prefix.self_attn.o_proj.weight"] =
            _qwen3_bf16_values((model.d_model, q_dim); seed=50 + layer)
        tensors["$prefix.self_attn.q_norm.weight"] =
            _qwen3_bf16_values((model.head_dim,); norm=true, seed=60 + layer)
        tensors["$prefix.self_attn.k_norm.weight"] =
            _qwen3_bf16_values((model.head_dim,); norm=true, seed=70 + layer)
        tensors["$prefix.post_attention_layernorm.weight"] =
            _qwen3_bf16_values((model.d_model,); norm=true, seed=80 + layer)
        tensors["$prefix.mlp.gate_proj.weight"] =
            _qwen3_bf16_values((model.mlp_hidden_dim, model.d_model); seed=90 + layer)
        tensors["$prefix.mlp.up_proj.weight"] =
            _qwen3_bf16_values((model.mlp_hidden_dim, model.d_model); seed=100 + layer)
        tensors["$prefix.mlp.down_proj.weight"] =
            _qwen3_bf16_values((model.d_model, model.mlp_hidden_dim); seed=110 + layer)
    end
    tensors["model.norm.weight"] = _qwen3_bf16_values(
        (model.d_model,);
        norm=true,
        seed=120,
    )
    model.tie_embeddings || (tensors["lm_head.weight"] = _qwen3_bf16_values(
        (model.vocab_size, model.d_model);
        seed=130,
    ))
    return tensors
...
function _qwen3_bf16_fixture_dir(directory; tie=false)
    write(
        joinpath(directory, "config.json"),
        JSON3.write(_qwen3_bf16_config(; tie_word_embeddings=tie)),
    )
    ...
    tensors = _qwen3_bf16_qwen_tensors(model)
    specs = [
        (; name, dtype="BF16", array=tensors[name])
        for name in sort!(collect(keys(tensors)))
    ]
    _qwen3_bf16_write_safetensors(joinpath(directory, "model.safetensors"), specs)
    return model, tensors
end

@testset "BF16 weight loading is bit-preserving" begin
    mktempdir() do directory
        _, tensors = _qwen3_bf16_fixture_dir(directory)
...
@testset "BF16 mixed-precision operator semantics" begin
    weight = BFloat16.(_qwen3_bf16_values((3, 4); seed=7))
    x = BFloat16.(reshape(_qwen3_bf16_values((4, 2); seed=3), 4, 2, 1))
    ...
    scale = BFloat16.(_qwen3_bf16_values((5,); norm=true, seed=2))
    h = BFloat16.(reshape(_qwen3_bf16_values((5, 2); seed=9), 5, 2, 1))
...
@testset "BF16 forward path on synthetic model" begin
    mktempdir() do directory
        model, _ = _qwen3_bf16_fixture_dir(directory; tie=false)
...
@testset "Qwen3 BF16 compute asset contract" begin
    fixture = JSON3.read(read(_QWEN3_BF16_ASSETS_PATH, String))
    qwen3_family = JSON3.read(read(_QWEN3_FAMILY_SPECS_PATH_FOR_QWEN3_BF16, String))
...
function _qwen3_bf16_integration(variant::Symbol, model_dir::AbstractString)
    fixture = JSON3.read(read(_QWEN3_BF16_ASSETS_PATH, String))
    entry = only(
...
for (variant, env_name) in (
    ...
    if haskey(ENV, env_name)
        @testset "Qwen3 $(variant) BF16 HuggingFace integration" begin
            _qwen3_bf16_integration(variant, ENV[env_name])
        end
    end
end
