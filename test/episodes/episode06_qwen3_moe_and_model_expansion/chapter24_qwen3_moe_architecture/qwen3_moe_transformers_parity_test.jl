...
@testset "Qwen3 MoE Transformers tiny checkpoint parity" begin
    ...
    @test length(streamed_cache.decode_blocks) == loaded.model.num_layers
    @test streamed_cache.decode_final_hidden ≈ _qwen3_moe_hf_layout(
        reference["final_hidden"][:, end:end, :],
    ) atol = 5.0f-6 rtol = 5.0f-6
    @test all(
    ...
    streamed_bf16 = stream_hf_qwen3_moe_forward(
        _QWEN3_MOE_TINY_PARITY_DIR,
        prompt_tokens;
        decode_token,
        max_seq_len=16,
    ...
    @test eltype(streamed_bf16.embedding) == BFloat16
    @test eltype(streamed_bf16.logits) == BFloat16
    @test eltype(streamed_bf16.decode_logits) == BFloat16
    @test streamed_bf16.compute_dtype === BFloat16
    @test argmax(vec(streamed_bf16.logits)) == argmax(vec(expected_prompt))
    @test argmax(vec(streamed_bf16.decode_logits)) == argmax(vec(expected_decode))
    @test all(
        length(active) == loaded.model.experts_per_token
        for active in streamed_bf16.decode_active_experts
    )
