...
function _gpt2_parity_config(; kwargs...)
    return merge(
        Dict{String,Any}(
...
function _gpt2_parity_causal_buffer(length)
...
function _gpt2_parity_values(shape, offset=0)
    return reshape(
...
function _gpt2_parity_tensors(model::GPTModel; source_max_seq_len=model.max_seq_len)
    d = model.d_model
    hidden = model.mlp_hidden_dim
    tensors = Dict{String,Any}(
        "wte.weight" => _gpt2_parity_values((model.vocab_size, d), 1),
        "wpe.weight" => _gpt2_parity_values((source_max_seq_len, d), 2),
        "ln_f.weight" => ones(Float32, d),
        "ln_f.bias" => zeros(Float32, d),
    )
    for layer in 0:(model.num_layers - 1)
        prefix = "h.$layer"
        tensors["$prefix.attn.bias"] = _gpt2_parity_causal_buffer(source_max_seq_len)
        tensors["$prefix.attn.c_attn.weight"] = _gpt2_parity_values((d, 3d), 10)
        tensors["$prefix.attn.c_attn.bias"] = _gpt2_parity_values((3d,), 20)
        tensors["$prefix.attn.c_proj.weight"] = _gpt2_parity_values((d, d), 30)
        tensors["$prefix.attn.c_proj.bias"] = _gpt2_parity_values((d,), 40)
        ...
        tensors["$prefix.mlp.c_fc.weight"] = _gpt2_parity_values((d, hidden), 50)
        tensors["$prefix.mlp.c_fc.bias"] = _gpt2_parity_values((hidden,), 60)
        tensors["$prefix.mlp.c_proj.weight"] = _gpt2_parity_values((hidden, d), 70)
        tensors["$prefix.mlp.c_proj.bias"] = _gpt2_parity_values((d,), 80)
    end
    return tensors
end
...
@testset "GPT-2 strict config and Conv1D mapping" begin
    mktempdir() do directory
        ...
        write(path, JSON3.write(_gpt2_parity_config()))
        ...
        write(bad_path, JSON3.write(_gpt2_parity_config(activation_function="gelu")))
        @test_throws ArgumentError load_hf_gpt2_config(bad_path)
        write(bad_path, JSON3.write(_gpt2_parity_config(extra_field=true)))
        @test_throws ArgumentError load_hf_gpt2_config(bad_path)

        model = GPTModel(config)
        tensors = _gpt2_parity_tensors(model)
...
@testset "learned position full/dynamic/static cache agreement" begin
    ...
    ps = load_hf_gpt2_parameters(model, _gpt2_parity_tensors(model))
...
if !isempty(model_dir) && !isempty(reference_dir)
    ...
    @info "Skipping GPT-2 HuggingFace parity integration; set LIFEAI_GPT2_MODEL_DIR and LIFEAI_GPT2_REFERENCE_DIR"
end
