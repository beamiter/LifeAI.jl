...
const _QWEN3_REAL_WEIGHTS_ASSETS_PATH = joinpath(
    @__DIR__,
    "fixtures",
    "qwen3_dense_real_weights",
    "assets.json",
)

const _QWEN3_FAMILY_SPECS_PATH_FOR_QWEN3_REAL_WEIGHTS = joinpath(
    @__DIR__,
    "..",
...
_qwen3_real_weights_is_sha256(value) = occursin(r"^[0-9a-f]{64}$", String(value))

@testset "Qwen3 real-weight asset contract" begin
    fixture = JSON3.read(read(_QWEN3_REAL_WEIGHTS_ASSETS_PATH, String))
    qwen3_family = JSON3.read(read(_QWEN3_FAMILY_SPECS_PATH_FOR_QWEN3_REAL_WEIGHTS, String))
    ...
    for entry in models
        ...
        for (name, file) in files
            @test _qwen3_real_weights_is_sha256(file["sha256"])
...
function _qwen3_real_weights_integration(variant::Symbol, model_dir::AbstractString)
    fixture = JSON3.read(read(_QWEN3_REAL_WEIGHTS_ASSETS_PATH, String))
    entry = only(
...
if haskey(ENV, "LIFEAI_QWEN3_1_7B_MODEL_DIR")
    @testset "Qwen3-1.7B HuggingFace integration" begin
        _qwen3_real_weights_integration(:qwen3_1_7b, ENV["LIFEAI_QWEN3_1_7B_MODEL_DIR"])
    end
end

if haskey(ENV, "LIFEAI_QWEN3_4B_MODEL_DIR")
    @testset "Qwen3-4B HuggingFace integration" begin
        _qwen3_real_weights_integration(:qwen3_4b, ENV["LIFEAI_QWEN3_4B_MODEL_DIR"])
    end
end
