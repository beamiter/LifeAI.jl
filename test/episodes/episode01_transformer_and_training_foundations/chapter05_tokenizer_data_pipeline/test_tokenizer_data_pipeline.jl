...
_tokenizer_pipeline_document(id, text) = TextDocument(
    id,
    text;
    source_id="tokenizer_pipeline-fixture",
    source_location="repository fixture",
    license="CC0-1.0",
)

function _tokenizer_pipeline_tree_isapprox(left, right; atol=1.0f-6, rtol=1.0f-5)
    ...
    elseif left isa NamedTuple
        ...
        return all(
            _tokenizer_pipeline_tree_isapprox(a, b; atol, rtol) for
    ...
    elseif isstructtype(typeof(left)) && fieldcount(typeof(left)) > 0
        return all(
            _tokenizer_pipeline_tree_isapprox(
                getfield(left, index),
...
@testset "tokenizer interfaces and artifacts" begin
    legacy_text = "你好，世界。\n你好，GPT！"
...
@testset "document split, train-only BPE, and boundary-safe loader" begin
    seed = 20260720
    placeholders = [_tokenizer_pipeline_document("doc-$index", "placeholder") for index in 1:4]
    preliminary = split_documents(placeholders; validation_size=1, seed)
    validation_id = only(preliminary.validation).id
    documents = TextDocument[
        _tokenizer_pipeline_document(
            placeholder.id,
    ...
    mktempdir() do directory
        artifact = save_dataset_artifact(
            directory,
            data;
            name="tokenizer_pipeline-chinese-fixture",
            version="1",
        )
...
@testset "byte-normalized evaluation, checkpoint, and generation" begin
    documents = [
        _tokenizer_pipeline_document("train-a", repeat("生命感来自连续观察与行动。", 60)),
        _tokenizer_pipeline_document("train-b", repeat("机器人保存经验并根据反馈调整。", 60)),
        _tokenizer_pipeline_document("validation", repeat("记忆让长期互动保持连续。", 60)),
    ]
    data = build_document_dataset(
        documents;
        tokenizer_type=:byte_bpe,
    ...
    mktempdir() do directory
        path = joinpath(directory, "tokenizer_pipeline.checkpoint")
        save_checkpoint(
            path,
            model,
            data.tokenizer,
            ...
            metadata=(; stage=5, dataset_fingerprint=data.split.fingerprint),
        )
        ...
        @test tokenizer_fingerprint(checkpoint.tokenizer) ==
              tokenizer_fingerprint(data.tokenizer)
        @test checkpoint.metadata.stage == 5
        ...
        @test _tokenizer_pipeline_tree_isapprox(
...
@testset "legacy v2 character checkpoint payload remains loadable" begin
