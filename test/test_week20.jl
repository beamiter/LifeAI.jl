...
@testset "frozen Week20 hardware evidence is self-consistent" begin
...
@testset "streamed compact tree is one-transfer ready" begin
    mktempdir() do directory
        _week16_fixture_dir(directory; tie=false)
...
function _week20_script_source(name::AbstractString)
    return read(joinpath(@__DIR__, "..", "scripts", name), String)
end

function _week20_required_match(pattern::Regex, source::AbstractString)
    result = match(pattern, source)
    @test result !== nothing
    return result
end

function _week20_last_match(pattern::Regex, source::AbstractString)
    result = nothing
...
@testset "Week20 XLA CLI acceptance contract is fail-closed" begin
    benchmark = _week20_script_source(
        "benchmark_qwen3_xla_deployment.jl",
    )
    chat = _week20_script_source("run_qwen3_xla_chat.jl")

    function allocator_fraction_default(source)
        result = _week20_required_match(
    ...
    pinned_reference = _week20_required_match(
        r"""const\s+WEEK20_CUDA_REFERENCE_SHA256\s*=\s*"([0-9a-f]{64})"""s,
        benchmark,
    )
    ...
    @test occursin(
        r"reference_sha256\s*==\s*WEEK20_CUDA_REFERENCE_SHA256\s*\|\|\s*error"s,
        benchmark,
    )
    @test occursin(
    ...
    frozen_cases = _week20_required_match(
    ...
    fail_closed = _week20_last_match(
        r"closed\s*\|\|\s*error\s*\(",
        benchmark,
    )
    report_write = _week20_last_match(r"JSON3\.pretty\(", benchmark)
