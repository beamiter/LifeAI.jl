...
function _tokenizer_pipeline_matrix_document(id, suffix)
    return TextDocument(
        id,
        repeat("生命感来自观察、记忆、反馈和行动。$suffix", 24);
        source_id="tokenizer_pipeline-matrix",
        source_location="repository test fixture",
        license="CC0-1.0",
    )
end

@testset "three-tokenizer end-to-end matrix" begin
    documents = [
        _tokenizer_pipeline_matrix_document("matrix-a", "甲"),
        _tokenizer_pipeline_matrix_document("matrix-b", "乙"),
        _tokenizer_pipeline_matrix_document("matrix-c", "丙"),
    ]
