...
const _QWEN3_FAMILY_SPECS_PATH = joinpath(
    @__DIR__,
    "fixtures",
    "qwen3_dense_family",
    "specs.json",
...
function _qwen3_family_untied_wide_tensors(model::GPTModel)
    tensors = Dict{String,Any}(
        "model.embed_tokens.weight" => _qwen3_family_values(
            (model.vocab_size, model.d_model),
            1,
        ),
        "model.norm.weight" => ones(Float32, model.d_model),
        "lm_head.weight" => _qwen3_family_values(
            (model.vocab_size, model.d_model),
            2,
        ),
    )
    ...
    for layer in 0:(model.num_layers - 1)
        ...
        tensors["$prefix.self_attn.q_proj.weight"] =
            _qwen3_family_values((query_dim, model.d_model), 10 + layer)
        tensors["$prefix.self_attn.k_proj.weight"] =
            _qwen3_family_values((kv_dim, model.d_model), 20 + layer)
        tensors["$prefix.self_attn.v_proj.weight"] =
            _qwen3_family_values((kv_dim, model.d_model), 30 + layer)
        tensors["$prefix.self_attn.o_proj.weight"] =
            _qwen3_family_values((model.d_model, query_dim), 40 + layer)
        ...
        tensors["$prefix.mlp.gate_proj.weight"] = _qwen3_family_values(
            (model.mlp_hidden_dim, model.d_model),
            50 + layer,
        )
        tensors["$prefix.mlp.up_proj.weight"] = _qwen3_family_values(
            (model.mlp_hidden_dim, model.d_model),
            60 + layer,
        )
        tensors["$prefix.mlp.down_proj.weight"] = _qwen3_family_values(
...
@testset "Qwen3 untied wide-attention weight and cache path" begin
    ...
    tensors = _qwen3_family_untied_wide_tensors(model)
