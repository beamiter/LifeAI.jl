...
const _WEEK15_ASSETS_PATH = joinpath(
    @__DIR__,
    "fixtures",
    "week15_qwen3_bf16_accel",
    "assets.json",
)

function _week15_row_major_values(array)
    values = Float32.(array)
    ndims(values) <= 1 && return vec(values)
    return vec(permutedims(values, Tuple(reverse(1:ndims(values)))))
end

function _week15_tensor_bytes(array)
    values = _week15_row_major_values(array)
    bits = UInt16[UInt16(reinterpret(UInt32, value) >> 16) for value in values]
    return collect(reinterpret(UInt8, bits))
end

function _week15_write_safetensors(path, tensors)
    header = Dict{String,Any}()
    data = UInt8[]
    offset = 0
    for name in sort!(collect(keys(tensors)))
        bytes = _week15_tensor_bytes(tensors[name])
...
function _week15_config(; kwargs...)
    return merge(
        Dict{String,Any}(
...
function _week15_values(shape; norm=false, seed=0)
    count = prod(shape)
    values = Float32[
...
function _week15_fixture_dir(directory; tie=false)
    write(
        joinpath(directory, "config.json"),
        JSON3.write(_week15_config(; tie_word_embeddings=tie)),
    )
    ...
    tensors["model.embed_tokens.weight"] = _week15_values(
    ...
    for layer in 0:(model.num_layers - 1)
        ...
        tensors["$prefix.input_layernorm.weight"] =
            _week15_values((model.d_model,); norm=true, seed=10 + layer)
        tensors["$prefix.self_attn.q_proj.weight"] =
            _week15_values((q_dim, model.d_model); seed=20 + layer)
        tensors["$prefix.self_attn.k_proj.weight"] =
            _week15_values((kv_dim, model.d_model); seed=30 + layer)
        tensors["$prefix.self_attn.v_proj.weight"] =
            _week15_values((kv_dim, model.d_model); seed=40 + layer)
        tensors["$prefix.self_attn.o_proj.weight"] =
            _week15_values((model.d_model, q_dim); seed=50 + layer)
        tensors["$prefix.self_attn.q_norm.weight"] =
            _week15_values((model.head_dim,); norm=true, seed=60 + layer)
        tensors["$prefix.self_attn.k_norm.weight"] =
            _week15_values((model.head_dim,); norm=true, seed=70 + layer)
        tensors["$prefix.post_attention_layernorm.weight"] =
            _week15_values((model.d_model,); norm=true, seed=80 + layer)
        tensors["$prefix.mlp.gate_proj.weight"] =
            _week15_values((model.mlp_hidden_dim, model.d_model); seed=90 + layer)
        tensors["$prefix.mlp.up_proj.weight"] =
            _week15_values((model.mlp_hidden_dim, model.d_model); seed=100 + layer)
        tensors["$prefix.mlp.down_proj.weight"] =
            _week15_values((model.d_model, model.mlp_hidden_dim); seed=110 + layer)
    end
    tensors["model.norm.weight"] = _week15_values(
        (model.d_model,);
        norm=true,
        seed=120,
    )
    model.tie_embeddings || (tensors["lm_head.weight"] = _week15_values(
        (model.vocab_size, model.d_model);
        seed=130,
    ))
    _week15_write_safetensors(joinpath(directory, "model.safetensors"), tensors)
    return model
end

@testset "BF16 batched matmul keeps F32 accumulation on CPU" begin
    a = BFloat16.(_week15_values((3, 4, 2); seed=5))
    b = BFloat16.(_week15_values((4, 2, 2); seed=8))
...
@testset "vectorized BF16 path is bitwise identical to the loop path" begin
    for tie in (false, true)
        mktempdir() do directory
            model = _week15_fixture_dir(directory; tie)
...
@testset "Qwen3 BF16 accelerated asset contract" begin
    fixture = JSON3.read(read(_WEEK15_ASSETS_PATH, String))
...
if any(haskey(ENV, name) for (_, name) in _WEEK15_CUDA_ENV_NAMES)

using CUDA

function _week15_cuda_integration(variant::Symbol, model_dir::AbstractString)
...
for (variant, env_name) in _WEEK15_CUDA_ENV_NAMES
    if haskey(ENV, env_name)
        @testset "Qwen3 $(variant) BF16 CUDA integration" begin
            _week15_cuda_integration(variant, ENV[env_name])
        end
    end
end
