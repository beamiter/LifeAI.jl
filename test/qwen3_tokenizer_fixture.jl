...
const QWEN3_TOKENIZER_FIXTURE_REGEX = raw"(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\p{L}\p{N}]?\p{L}+|\p{N}| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+"

function qwen3_tokenizer_fixture_payloads()
    ...
    tokenizer = Dict{String,Any}(
        ...
        "pre_tokenizer" => Dict(
            ...
            "pretokenizers" => Any[
                Dict(
                    "type" => "Split",
                    "pattern" => Dict("Regex" => QWEN3_TOKENIZER_FIXTURE_REGEX),
                    "behavior" => "Isolated",
                    "invert" => false,
                ),
                Dict(
    ...
    tokenizer_config = Dict{String,Any}(
        ...
        "bos_token" => nothing,
        "chat_template" => "qwen3-tokenizer-fixture-template",
...
function write_qwen3_tokenizer_fixture(directory; payloads=qwen3_tokenizer_fixture_payloads())
    mkpath(directory)
