...
const _WEEK13_ASSETS_PATH = joinpath(
    @__DIR__,
    "fixtures",
    "week13_qwen3_streamed_large_weights",
    "assets.json",
)

const _WEEK11_SPECS_PATH_FOR_WEEK13 = joinpath(
    @__DIR__,
    "fixtures",
...
function _week13_row_major_values(array)
    values = Float32.(array)
    ndims(values) <= 1 && return vec(values)
    return vec(permutedims(values, Tuple(reverse(1:ndims(values)))))
end

function _week13_tensor_bytes(array, dtype::String)
    values = _week13_row_major_values(array)
    if dtype == "F32"
...
function _week13_write_safetensors(path, specs)
    header = Dict{String,Any}()
    data = UInt8[]
    offset = 0
    for spec in specs
        bytes = _week13_tensor_bytes(spec.array, spec.dtype)
...
function _week13_config(; kwargs...)
    return merge(
        Dict{String,Any}(
...
function _week13_values(shape; norm=false, seed=0)
    count = prod(shape)
    values = Float32[
...
function _week13_qwen_tensors(model::GPTModel)
    tensors = Dict{String,Any}()
    tensors["model.embed_tokens.weight"] = _week13_values(
    ...
    for layer in 0:(model.num_layers - 1)
        ...
        tensors["$prefix.input_layernorm.weight"] =
            _week13_values((model.d_model,); norm=true, seed=10 + layer)
        tensors["$prefix.self_attn.q_proj.weight"] =
            _week13_values((q_dim, model.d_model); seed=20 + layer)
        tensors["$prefix.self_attn.k_proj.weight"] =
            _week13_values((kv_dim, model.d_model); seed=30 + layer)
        tensors["$prefix.self_attn.v_proj.weight"] =
            _week13_values((kv_dim, model.d_model); seed=40 + layer)
        tensors["$prefix.self_attn.o_proj.weight"] =
            _week13_values((model.d_model, q_dim); seed=50 + layer)
        tensors["$prefix.self_attn.q_norm.weight"] =
            _week13_values((model.head_dim,); norm=true, seed=60 + layer)
        tensors["$prefix.self_attn.k_norm.weight"] =
            _week13_values((model.head_dim,); norm=true, seed=70 + layer)
        tensors["$prefix.post_attention_layernorm.weight"] =
            _week13_values((model.d_model,); norm=true, seed=80 + layer)
        tensors["$prefix.mlp.gate_proj.weight"] =
            _week13_values((model.mlp_hidden_dim, model.d_model); seed=90 + layer)
        tensors["$prefix.mlp.up_proj.weight"] =
            _week13_values((model.mlp_hidden_dim, model.d_model); seed=100 + layer)
        tensors["$prefix.mlp.down_proj.weight"] =
            _week13_values((model.d_model, model.mlp_hidden_dim); seed=110 + layer)
    end
    tensors["model.norm.weight"] = _week13_values(
        (model.d_model,);
        norm=true,
        seed=120,
    )
    model.tie_embeddings || (tensors["lm_head.weight"] = _week13_values(
        (model.vocab_size, model.d_model);
        seed=130,
    ))
    return tensors
...
function _week13_specs(tensors, names; dtype="BF16")
    return [(; name, dtype, array=tensors[name]) for name in names]
end

function _week13_write_sharded(directory, tensors; dtype="BF16")
    ...
    for (shard, shard_names) in shards
        _week13_write_safetensors(
            joinpath(directory, shard),
            _week13_specs(tensors, shard_names; dtype),
        )
...
function _week13_fixture_dir(directory; tie=false, dtype="BF16")
    write(
        joinpath(directory, "config.json"),
        JSON3.write(_week13_config(; tie_word_embeddings=tie)),
    )
    ...
    tensors = _week13_qwen_tensors(model)
    _week13_write_sharded(directory, tensors; dtype)
    return model, tensors
end

@testset "streamed safetensors reader strictness" begin
    mktempdir() do directory
        model, tensors = _week13_fixture_dir(directory)
...
function _week13_streamed_equals_in_memory(directory, model)
...
@testset "streamed forward is bitwise identical to in-memory" begin
    mktempdir() do directory
        model, _ = _week13_fixture_dir(directory; tie=false, dtype="BF16")
        @test !model.tie_embeddings
        @test model.num_heads * model.head_dim > model.d_model
        _week13_streamed_equals_in_memory(directory, model)
    end

    mktempdir() do directory
        model, _ = _week13_fixture_dir(directory; tie=true, dtype="F32")
        @test model.tie_embeddings
        _week13_streamed_equals_in_memory(directory, model)
    end

    mktempdir() do directory
        model, tensors = _week13_fixture_dir(directory; tie=true)
        tensors["lm_head.weight"] = _week13_values(
            (model.vocab_size, model.d_model);
            seed=131,
        )
        _week13_write_sharded(directory, tensors)
...
@testset "Qwen3 streamed large-weight asset contract" begin
    fixture = JSON3.read(read(_WEEK13_ASSETS_PATH, String))
    week11 = JSON3.read(read(_WEEK11_SPECS_PATH_FOR_WEEK13, String))
...
function _week13_integration(variant::Symbol, model_dir::AbstractString)
    fixture = JSON3.read(read(_WEEK13_ASSETS_PATH, String))
    entry = only(
        e for e in fixture["models"] if Symbol(String(e["variant"])) == variant
    )
    reference_dir = joinpath(model_dir, "lifeai-references", "week13-parity")
...
for (variant, env_name) in (
    ...
    if haskey(ENV, env_name)
        @testset "Qwen3 $(variant) streamed HuggingFace integration" begin
            _week13_integration(variant, ENV[env_name])
        end
    end
end
