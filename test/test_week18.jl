...
const _WEEK18_FIXTURE_DIR = joinpath(
    @__DIR__,
    "fixtures",
    "week18_qwen3_activation_calibration",
)
const _WEEK18_ASSETS_PATH = joinpath(_WEEK18_FIXTURE_DIR, "assets.json")

function _week18_weighted_error(weight, quantized, moment)
...
@testset "Qwen3-14B activation calibration asset contract" begin
    assets = JSON3.read(read(_WEEK18_ASSETS_PATH, String))
    for (filename, expected) in pairs(assets["plan_sha256"])
        path = joinpath(_WEEK18_FIXTURE_DIR, String(filename))
    ...
    plan_fixture = JSON3.read(read(
        joinpath(_WEEK18_FIXTURE_DIR, "plan_mixed_24g_activation.json"),
        String,
    ))
    ...
    baselines = hardware["week17_baselines"]
...
@testset "activation-weighted INT4 scale calibration" begin
    ...
    @test _week18_weighted_error(weight, activation_mse, moment) <
        _week18_weighted_error(weight, maxabs, moment)
    @test _week18_weighted_error(weight, activation_mse, moment) <
        _week18_weighted_error(weight, weight_mse, moment)
...
@testset "streamed activation collection drives both quantizers" begin
    mktempdir() do directory
        ...
        @test calibration.source == "week18-test"
        ...
        @test _week17_quantized_equal(in_memory, streamed.parameters)
    ...
    mktempdir() do directory
        model = _week16_fixture_dir(directory; tie=true)
