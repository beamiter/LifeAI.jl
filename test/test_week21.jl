...
const _WEEK21_MODEL = "Qwen/Qwen3-8B"

function _week21_fake_service(;
    delay_seconds=0.0,
...
function _week21_generate_request(; kwargs...)
    return _week21_request(
        "POST",
        "/api/generate";
        body=_week21_payload(; kwargs...),
        content_type="application/json; charset=utf-8",
    )
end

_week21_json(response) =
    JSON3.read(String(response.body), Dict{String,Any})

function _week21_error_code(response)
    return String(_week21_json(response)["code"])
end

@testset "Week 21 service loads once and serves Ollama-compatible JSON" begin
    fixture = _week21_fake_service()
    service = fixture.service
    @test fixture.load_calls[] == 1

    health = qwen3_xla_http_handler(
        service,
        _week21_request("GET", "/healthz?probe=1"),
    )
    @test health.status == 200
    @test HTTP.header(health, "Content-Type") ==
        "application/json; charset=utf-8"
    health_json = _week21_json(health)
    @test health_json["status"] == "ok"
    @test health_json["ready"] === true
    @test health_json["model"] == _WEEK21_MODEL
    @test health_json["load_count"] == 1

    response = qwen3_xla_http_handler(
        service,
        _week21_generate_request(stream=false, num_predict=16),
    )
    @test response.status == 200
    @test HTTP.header(response, "Content-Type") ==
        "application/json; charset=utf-8"
    object = _week21_json(response)
    @test object["model"] == _WEEK21_MODEL
...
@testset "Week 21 NDJSON streams only valid UTF-8 pieces" begin
    fixture = _week21_fake_service()
    response = qwen3_xla_http_handler(
        fixture.service,
        _week21_generate_request(stream=true),
    )
    @test response.status == 200
    @test HTTP.header(response, "Content-Type") ==
...
@testset "Week 21 request gates fail closed with JSON errors" begin
    fixture = _week21_fake_service()
    service = fixture.service

    missing = qwen3_xla_http_handler(
        service,
        _week21_request("GET", "/missing"),
    )
    @test missing.status == 404
    @test _week21_error_code(missing) == "not_found"

    wrong_method = qwen3_xla_http_handler(
        service,
        _week21_request("GET", "/api/generate"),
    )
    @test wrong_method.status == 405
    @test _week21_error_code(wrong_method) == "method_not_allowed"
    ...
    wrong_type = qwen3_xla_http_handler(
        ...
        _week21_request(
            "POST",
            "/api/generate";
            body=_week21_payload(),
            content_type="text/plain",
        ),
    )
    @test wrong_type.status == 415
    @test _week21_error_code(wrong_type) == "unsupported_media_type"
    ...
    @test _week21_error_code(malformed) == "invalid_json"

    mismatch = qwen3_xla_http_handler(
        service,
        _week21_generate_request(model="another-model"),
    )
    @test mismatch.status == 400
    @test _week21_error_code(mismatch) == "model_mismatch"

    templated = qwen3_xla_http_handler(
        service,
        _week21_generate_request(raw=false),
    )
    @test templated.status == 400
    @test _week21_error_code(templated) == "raw_required"

    sampling = qwen3_xla_http_handler(
        service,
        _week21_generate_request(temperature=0.8),
    )
    @test sampling.status == 400
    @test _week21_error_code(sampling) == "sampling_unsupported"

    context_mismatch = qwen3_xla_http_handler(
        service,
        _week21_generate_request(num_ctx=32),
    )
    @test context_mismatch.status == 400
    @test _week21_error_code(context_mismatch) == "context_mismatch"

    overflow = qwen3_xla_http_handler(
        service,
        _week21_generate_request(prompt="overflow", num_predict=16),
    )
    @test overflow.status == 400
    @test _week21_error_code(overflow) == "context_length_exceeded"

    unknown = qwen3_xla_http_handler(
        service,
        _week21_generate_request(extra=Dict("format" => "json")),
    )
    @test unknown.status == 400
    @test _week21_error_code(unknown) == "unsupported_field"

    small = _week21_fake_service(max_body_bytes=32).service
    oversized = qwen3_xla_http_handler(
        small,
        _week21_generate_request(),
    )
    @test oversized.status == 413
    @test _week21_error_code(oversized) == "body_too_large"
...
@testset "Week 21 static K/V access is single-flight" begin
    fixture = _week21_fake_service(delay_seconds=0.04)
    responses = Vector{HTTP.Response}(undef, 2)
    @sync for index in eachindex(responses)
        @async responses[index] = qwen3_xla_http_handler(
            fixture.service,
            _week21_generate_request(prompt="p$index"),
        )
    end
...
@testset "Week 21 generation failures release the lock" begin
    fixture = _week21_fake_service()
    failed = qwen3_xla_http_handler(
        fixture.service,
        _week21_generate_request(prompt="boom"),
    )
    @test failed.status == 500
    @test _week21_error_code(failed) == "generation_failed"

    recovered = qwen3_xla_http_handler(
        fixture.service,
        _week21_generate_request(prompt="ok"),
    )
    @test recovered.status == 200
    @test _week21_json(recovered)["response"] == "你!"
    status = qwen3_xla_service_status(fixture.service)
    @test status.requests.total == 2
...
@testset "Week 21 server defaults remain loopback-only" begin
    source = read(
...
@testset "frozen Week 21 hardware evidence is self-consistent" begin
    evidence_dir = joinpath(
        @__DIR__,
        "..",
        "benchmark_results",
        "week21",
...
if lowercase(get(ENV, "LIFEAI_TEST_HTTP_SOCKET", "false")) in
        ("1", "true", "yes")
    @testset "Week 21 loopback HTTP transport" begin
        fixture = _week21_fake_service()
        ...
        try
            ...
            @test health.status == 200
            @test _week21_json(health)["load_count"] == 1

            generated = HTTP.post(
                "http://127.0.0.1:$port/api/generate",
                ["Content-Type" => "application/json"],
                _week21_payload(stream=true),
            )
