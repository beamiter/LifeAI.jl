...
const _WEEK17_ASSETS_PATH = joinpath(
    @__DIR__,
    "fixtures",
    "week17_qwen3_calibrated_int4",
    "assets.json",
)

function _week17_quantized_equal(left, right)
    ...
    elseif left isa NamedTuple
        ...
        return all(
            _week17_quantized_equal(getfield(left, key), getfield(right, key))
            for key in keys(left)
        )
    elseif left isa Tuple
        return length(left) == length(right) &&
            all(_week17_quantized_equal.(left, right))
    end
    return left == right
end
...
@testset "INT4 reconstruction-MSE calibration" begin
    ...
    # Week 16's default call remains exactly max-abs RTN.
...
@testset "one plan drives in-memory and streamed quantization" begin
    mktempdir() do directory
        ...
        # Legacy Week 16 arguments resolve to the same plan and tensor values.
        ...
        @test _week17_quantized_equal(legacy.parameters, planned.parameters)
    end
...
@testset "Qwen3-14B frozen tensor-byte budgets" begin
    fixture = JSON3.read(read(_WEEK17_ASSETS_PATH, String))
    for (filename, expected) in pairs(fixture["plan_sha256"])
        path = joinpath(dirname(_WEEK17_ASSETS_PATH), String(filename))
