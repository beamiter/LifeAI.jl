...
_modern_gpt_components_all_finite(::Nothing) = true
_modern_gpt_components_all_finite(x::Number) = isfinite(x)
_modern_gpt_components_all_finite(x::AbstractArray) = all(isfinite, x)
_modern_gpt_components_all_finite(x::NamedTuple) = all(_modern_gpt_components_all_finite, values(x))
_modern_gpt_components_all_finite(x::Tuple) = all(_modern_gpt_components_all_finite, x)
_modern_gpt_components_all_finite(_) = true

function _modern_gpt_components_tree_isapprox(left::NamedTuple, right::NamedTuple; kwargs...)
    keys(left) == keys(right) || return false
    return all(
        _modern_gpt_components_tree_isapprox(l, r; kwargs...) for
        (l, r) in zip(values(left), values(right))
    )
end

function _modern_gpt_components_tree_isapprox(left::Tuple, right::Tuple; kwargs...)
...
function _modern_gpt_components_tree_isapprox(left::AbstractArray, right::AbstractArray; kwargs...)
    return isapprox(left, right; kwargs...)
end

_modern_gpt_components_tree_isapprox(left, right; kwargs...) = left == right

function _modern_gpt_components_dense_reference(ps, x)
...
function _modern_gpt_components_legacy_config(config)
    return (;
...
@testset "RMSNorm reference, dtype, and gradients" begin
    ...
    @test _modern_gpt_components_all_finite(grad_x)
    @test _modern_gpt_components_all_finite(grad_ps)
...
@testset "SwiGLU reference, width, parameters, and gradients" begin
    ...
    gate = _modern_gpt_components_dense_reference(ps.gate_proj, x)
    up = _modern_gpt_components_dense_reference(ps.up_proj, x)
    hidden = swish.(gate) .* up
    reference = _modern_gpt_components_dense_reference(ps.down_proj, hidden)
...
@testset "Legacy defaults and version-stable GPT config" begin
    ...
    @test _modern_gpt_components_tree_isapprox(default_ps, explicit_ps; atol=0, rtol=0)
    @test _modern_gpt_components_tree_isapprox(default_st, explicit_st; atol=0, rtol=0)
...
@testset "Five-configuration train and KV-cache correctness matrix" begin
    ...
    for (index, (name, options)) in enumerate(pairs(configurations))
        @testset "$(name)" begin
            ...
            @test _modern_gpt_components_all_finite(gradients)
        end
    end
end
...
@testset "Modern checkpoint round-trip, resume, generation, and v1 migration" begin
    ...
    mktempdir() do directory
        ...
        @test checkpoint.metadata == (; stage=4)
