...
- 默认套件 `4953 / 4953` 通过，Week 16 `78 / 78`、Week 17 最终专项
...
- ModelScope 下载的 5 个配置/tokenizer/index 文件与 8 个权重分片
  ...
  Torch 2.7.1+cpu，reference checksum 已冻结进 Week 17 fixture。
...
- **完成了什么**：完成校准式 INT4 scale search、统一的层/投影/LM-head
  ...
  4090 D 对照。
- **验证证据**：默认 `4953 / 4953`；Week 17 `83 / 83`；模型/reference
...
- **是否满足 Close 条件**：满足。算法不劣于候选 baseline 的局部契约、
  ...
  均已有证据；质量结论按实测写成负结果。
- **带到下一 Week 的问题**：若继续量化，优先做 activation-aware
