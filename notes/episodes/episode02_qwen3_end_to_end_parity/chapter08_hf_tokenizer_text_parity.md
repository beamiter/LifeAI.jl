# Chapter 08 — HuggingFace Qwen3 Tokenizer and Text-to-Text Parity
...
> 关闭记录：2026-07-22
>
> 依赖基线：[`Chapter 07 — HuggingFace Weight Loading and Qwen3 Logits Parity`](../../episodes/episode02_qwen3_end_to_end_parity/chapter07_hf_weight_loading.md) 已 Closed。
>
> 长期目标：本 Chapter 是「复现 Qwen3 → HF 权重加载 → 推理验证」三阶段计划的第三步。Chapter 06 已完成结构 parity，Chapter 07 已完成真实权重与 token-id→logits parity；Chapter 08 导入 HF tokenizer，并把验证边界推进到 text→text。

## 已确认的执行边界

1. **参照对象**：继续使用 Chapter 07 验证过的 `Qwen/Qwen3-0.6B`，模型 revision、tokenizer 文件 revision 与 reference 版本必须一起记录，避免“权重来自一个 revision、tokenizer 来自另一个 revision”。
...
4. **Chat 范围**：支持 Qwen3 常规 system/user/assistant 消息、`add_generation_prompt` 与 thinking 开关所需的确定性渲染；不在本 Chapter 实现通用 Jinja 引擎，也不承诺任意第三方 chat template。工具调用和 tool-role 消息留给后续 agent/tool 阶段。
...
Chapter 07 已证明：只要输入 token ids 相同，模型各层 hidden states、logits 和单步 cache decode 都能在明确容差内对齐。Chapter 08 因此不再猜测模型结构，而是把输入输出边界逐层拆开：Unicode 文本 → regex 分段 → UTF-8 bytes → byte-unicode symbols → BPE merges → added/special tokens → 1-based ids，以及生成 ids → bytes → 文本。首次分叉必须能定位到具体阶段。
...
| XLA 与全量回归 | 工程 | host tokenizer + XLA static decode smoke、默认/显式测试记录 | Chapter 01—07 默认测试通过；显式 XLA 套件通过；legacy tokenizer 无回归 | 已完成 |
...
- HF 0-based fixture 与 LifeAI 1-based API 只在边界显式转换；tokenizer 输出可直接进入 Chapter 07 模型，vocabulary 最小/最大 id 均有测试。
...
- 默认测试、显式 XLA 测试和真实 Qwen3 integration 全部通过，测试数、命令、版本、fixture 文本和结果写入本 Chapter 实施记录。
...
- **要验证的假设**：Chapter 05 的 byte/BPE 抽象可以复用核心 merge 思路，但 HF imported tokenizer 需要独立的 vocabulary、added-token 与 pre-tokenization 契约；Chapter 07 logits parity 足以让固定 prompt 的 greedy 多步 token 序列持续一致。
...
- **看似同为 byte-BPE，边界仍可完全不同**：Chapter 05 的 trainable byte-BPE 与 HF imported BPE 在 regex 分段、byte symbols、added tokens 和 id 排列上都有独立语义，不能只替换 vocabulary 就宣称兼容。
...
- **chat template 范围膨胀**：官方 template 同时包含 thinking 与 tools 分支。本 Chapter 冻结无 tools 的常规消息子集，对超出范围的输入明确拒绝，避免悄悄近似官方行为。
...
- 共 6 个生成 step 的下一 token argmax 全部一致。raw 四步 max-abs 为 `1.67e-5`—`1.76e-5`；chat 两步为 `5.054474e-5` / `4.887581e-5`。全局 max-abs 为 `5.054474e-5`，各 step mean-abs 的平均为 `4.507615e-6`，低于复用 Chapter 07 的 logits 容差 `atol=5e-3, rtol=5e-4`。
...
- 默认 `Pkg.test()`：4086 / 4086 通过，其中 Chapter 08 离线专项 61 / 61。
- `LIFEAI_TEST_XLA=true Pkg.test()`：默认套件通过，Reactant/XLA 52 / 52；Chapter 08 新增 host tokenizer → XLA static generation 3 / 3。
...
- **是否满足 Close 条件**：是。约定范围内全部 Close 条件已完成，并于 2026-07-22 关闭 Chapter 08。
- **带到下一 Chapter 的问题**：Qwen3 结构→权重→文本复现已完成；下一阶段可转向最小 agent loop、对话状态、工具接口与 replay，但当前不自动 Open 新 Chapter。
