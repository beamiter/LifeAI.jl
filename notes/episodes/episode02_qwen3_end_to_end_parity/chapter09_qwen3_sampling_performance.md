# Chapter 09 — Qwen3 Sampling Fidelity and Real Inference Performance
...
> 关闭记录：2026-07-23
>
> 依赖基线：[`Chapter 08 — HuggingFace Qwen3 Tokenizer and Text-to-Text Parity`](../../episodes/episode02_qwen3_end_to_end_parity/chapter08_hf_tokenizer_text_parity.md) 已 Closed。
>
> 近期主线：继续复现经典/SOTA 模型与论文，以可定位的数值验证和可重复的性能实验积累 LifeAI.jl 的模型组建、推理与框架能力。

## 核心问题与结论

> 在 Chapter 06—08 已完成 Qwen3-0.6B 结构、权重、tokenizer 与 greedy text-to-text parity 后，能否进一步严格复现官方实际推荐的 temperature → top-k → top-p 采样语义，并以长位置 correctness 和真实模型 benchmark 回答“结果是否对、代价是多少、瓶颈在哪里”？

Qwen3 官方 `generation_config.json` 默认启用采样，而不是 greedy：thinking 模式使用 `temperature=0.6`、`top_k=20`、`top_p=0.95`。Chapter 08 的 greedy 对齐适合隔离模型数值正确性，但不能代替真实推荐生成路径。Chapter 09 因此把验收推进到三层：过滤后的候选 mask / logits / probabilities、固定随机流下的逐 token 结果、真实 Qwen3-0.6B 的 prefill/decode 时间与 cache 内存。
...
1. **参照对象不漂移**：仍使用 Chapter 07/08 冻结的 `Qwen/Qwen3-0.6B` revision `c1899de289a04d12100db370d81485cdf75e47ca`、Transformers 4.51.0 和同 revision tokenizer/generation config。
...
4. **greedy 兼容不变**：`generate_hf_text` 默认仍为 `strategy=:greedy`，避免 Chapter 08 调用静默变成随机；`strategy=:config` 才采用官方文件，`:sample` 允许显式覆盖参数。
5. **dtype 表述严格**：当前官方 BF16 safetensors 被解码为 Float32 参数并以 Float32 计算。本 Chapter 不把它表述为 native BF16 inference；原生 BF16/CUDA kernel 是后续独立工作。
...
7. **性能证据分层**：correctness、CPU benchmark、CUDA/XLA benchmark 是不同证据。CPU 是本 Chapter 必做；加速器结果只有在硬件/后端实际可用且完成同步计时后才记录。
...
# 明确保持 Chapter 08 的 deterministic greedy。
...
- 默认测试、Chapter 09 opt-in integration 通过；加速器是否完成必须据实单列。
...
- native BF16 牵涉参数类型、CPU/BLAS、CUDA/XLA kernel 和归一化数值策略，不能作为一个 loader 参数草率完成；先用本 Chapter 性能证据决定后续专项。
- Qwen3 MoE、其他 dense 尺寸、量化和训练配方仍不在本 Chapter 范围。完成 0.6B 推理 fidelity 后再选择 MoE 或下一经典模型，避免范围无限扩张。

## 实验与过程记录

### 2026-07-22：Open 与第一批实现

- Chapter 08 Close commit `bd08705` 已推送到 `origin/main`，随后开启本 Chapter；未跟踪的 `artifacts/` 未被纳入提交。
...
- 完整默认测试 `4135 / 4135` 通过，其中 Chapter 09 离线专项 `46 / 46`，Chapter 03 benchmark contract 新增 3 项 raw-sample 测试；本次未重跑 XLA 专项，因为尚未新增 XLA sampling kernel 或真实加速器路径。
...
- 最终默认测试 `4156 / 4156`、真实 sampled integration `86 / 86`、显式 Reactant-XLA 专项 `52 / 52` 全部通过。默认 Chapter 09 专项由 `46` 项增至 `67` 项，新增的 21 项来自独立 HF RoPE fixture。
...
- **没有完成及原因**：没有 Chapter 09 Close 阻塞项。native BF16、量化、完整 40K 真实模型 dense forward、其他尺寸与 MoE 从一开始就不属于本 Chapter 范围，不能从本次 Float32 结果外推。
...
- **是否满足 Close 条件**：是。所有预先定义的 Close 条件均有代码、fixture、测试或原始 benchmark 证据，Chapter 09 于 2026-07-23 Closed。
- **带到下一 Chapter 的问题**：在近期“持续复现经典/SOTA 模型和论文”的主线下，下一步可在 native BF16/量化性能专项、Qwen3 MoE，或另一类经典架构复现之间选择；尚未自动 Open Chapter 10。
