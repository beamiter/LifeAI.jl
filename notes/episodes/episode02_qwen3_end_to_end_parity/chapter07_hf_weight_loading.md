# Chapter 07 — HuggingFace Weight Loading and Qwen3 Logits Parity
...
> 关闭记录：2026-07-22
>
> 依赖基线：[`Chapter 06 — GQA, QK-Norm and Qwen3 Structural Parity`](../../episodes/episode02_qwen3_end_to_end_parity/chapter06_gqa_qwen3_parity.md) 已 Closed。
>
> 长期目标：本 Chapter 是「复现 Qwen3 → HF 权重加载 → 推理验证」三阶段计划的第二步。Chapter 06 已完成结构 parity；Chapter 07 接入真实权重并完成 token-id→logits 数值对齐；Chapter 08 再导入 HF tokenizer，完成 text→text 端到端验证。
...
Chapter 06 已证明结构 shape、GQA head 路由和 QK-Norm 位置一致，但“结构同名”不等于“数值同构”。本 Chapter 必须把 RoPE 配对、tensor 存储顺序、embedding 索引基准、tied lm head 与 cache 绝对位置逐项钉死；最终验收依据是逐层数值证据，而不是成功读到权重或只对上 shape。
...
| checkpoint 与回归 | 工程 | `rope_style` config 迁移、加载后 checkpoint round-trip | Chapter 01—06 默认测试通过；旧 checkpoint logits 不变 | 已完成 |
| 内存与运行记录 | 工程 | BF16→F32 加载峰值、模型参数量、首次/稳态推理记录 | 不保留无必要的完整权重副本；结果写入本 Chapter 实施记录 | 已完成 |
...
- **要验证的假设**：Chapter 06 结构 parity 后无需改动 block 计算顺序；解决 RoPE/布局/索引差异即可使 Qwen3-0.6B logits 对齐；Float32 计算足以把 BF16 权重的跨框架误差控制在稳定容差内。

## 非目标

- 不导入 `tokenizer.json`，不承诺任意文本到 token ids 的 HF 一致性（Chapter 08）。
...
- 默认 `Pkg.test()`：4025 / 4025 通过，其中 Chapter 07 离线专项 54 / 54。
...
- **没有完成及原因**：未导入 HF tokenizer，也未提交真实权重/reference 大文件；前者按计划属于 Chapter 08，后者由仓库体积与默认离线测试边界决定。
...
- **是否满足 Close 条件**：是。全部约定条件已完成并于 2026-07-22 关闭 Chapter 07。
- **带到下一 Chapter 的问题**：HF tokenizer 导入与 text→text 端到端一致性（Chapter 08）；Chapter 08 尚未 Open。
