# Chapter 06 — GQA, QK-Norm and Qwen3 Structural Parity

> 状态：Closed
>
> 关闭记录：2026-07-22
>
> 依赖基线：[`Chapter 05 — Versioned Tokenizers and Chinese Data Pipeline`](../../episodes/episode01_transformer_and_training_foundations/chapter05_tokenizer_data_pipeline.md) 已 Closed。
>
> 长期目标：本 Chapter 是「复现 Qwen3 系列并从 HuggingFace 加载权重验证」三阶段计划的第一步。Chapter 06 完成结构 parity，Chapter 07 计划完成 safetensors 权重加载与 logits 对齐，Chapter 08 计划完成 HF tokenizer 导入与 text→text 端到端验证。

## 已确认的执行边界

1. **范围**：本 Chapter 聚焦 Grouped-Query Attention、QK-Norm 和 Qwen3 dense 结构所需的配置能力；不加载真实 HuggingFace 权重，不导入 HF tokenizer，不加入 MoE、量化、FlashAttention 或分布式训练。
2. **兼容策略**：`n_kv_heads` 缺省等于 `n_heads`，`use_qk_norm` 缺省为 `false`；现有全部调用、Chapter 03 / 04 / 05 checkpoint 和默认训练示例在缺省配置下行为与 logits 完全不变。
...
5. **对照口径**：GQA 的收益以 KV cache 内存占用和 decode 吞吐衡量，quality 对照沿用 Chapter 05 的 bits-per-byte 与固定 seed 口径；不为 GQA 调整其他超参数。
6. **参考架构**：以 Qwen3-0.6B 的结构字段为 parity 目标（GQA 16Q/8KV、head_dim=128 独立于 d_model、QK-Norm、RMSNorm、SwiGLU、no bias、rope_theta=1e6、tied embedding）；本 Chapter 用缩小的同构配置验证，不追求真实规模。
...
Chapter 04 已经证明结构开关可以独立配置和对照，Chapter 05 冻结了文本入口。Chapter 06 把结构对齐到 Qwen3：这是权重加载（Chapter 07）的前置条件——只有每个模块的形状、归一化位置和计算顺序与 Qwen3 一致，safetensors 权重才有落点。因此本 Chapter 的验收核心是结构正确性与兼容性，而不是训练质量。

## Qwen3 dense 结构对照

| 结构要素 | Qwen3 dense | LifeAI.jl 现状 | 本 Chapter 动作 |
| --- | --- | --- | --- |
| RMSNorm (pre-norm) | ✓ eps=1e-6 | Chapter 04 已有 | 校验 eps 可配 |
| SwiGLU MLP | ✓ | Chapter 04 已有 | 无 |
| RoPE theta=1e6 | ✓ | `rope_theta` 可配 | 无 |
| head_dim 独立于 d_model | ✓ (128) | `head_dim` 可配 | 无 |
| attention 无 bias | ✓ | `use_bias=false` | 无 |
| tied embedding | ✓ (0.6B/1.7B/4B) | Chapter 04 已有 | 无 |
| **GQA** | ✓ 16Q/8KV | 无 | **实现** |
...
- 为 Chapter 07 预留：`gpt_config` 字段与 HF `config.json` 字段（`num_key_value_heads`、`head_dim`、`rope_theta`、`rms_norm_eps`、`tie_word_embeddings` 等）的对应关系写入文档，作为权重加载的契约草案。
...
- Chapter 03 / 04 / 05 checkpoint 可恢复且 logits 不变；含 GQA + QK-Norm 的新 checkpoint 完成 round-trip / resume / cached generation。
...
- **要验证的假设**：GQA 在 `n_kv_heads==n_heads` 时可精确退化；cache 内存按 `n_kv_heads / n_heads` 缩减且 decode 吞吐可测量提升；结构 parity 完成后 Chapter 07 的权重加载不再需要改动模块计算顺序。

## 非目标

- 不下载或加载真实 HuggingFace 权重（Chapter 07）。
- 不实现 safetensors / bfloat16 读取或 HF 参数名映射（Chapter 07）。
- 不导入 HF tokenizer.json，不做 GPT-2 byte↔unicode 映射或 regex pre-tokenization（Chapter 08）。
...
- QK-Norm 的位置（RoPE 前 vs 后）和作用维度（per-head vs 全宽）在不同模型谱系中不一致；必须按 Qwen3 语义固定并写入测试，否则 Chapter 07 权重对不上。
...
- tiny 配置下 GQA 的 quality 影响不可外推；本 Chapter 只对 cache 内存和吞吐下结论，quality 差异记录但不排名。
...
8. **HF 契约草案**：`notes/../../qwen3_hf_config_mapping.md` 覆盖 Qwen3-0.6B 全部结构字段、权重名映射与歧义项；过程中确认一个 Chapter 07 关键风险：HF Qwen3 RoPE 使用 rotate_half（前半/后半配对），LifeAI `apply_rope` 使用相邻配对（interleaved），两者不等价，权重加载时必须做模式适配或 head_dim 维 permutation。
...
- **验证证据**：2026-07-22 默认测试 3971 / 3971 通过（Chapter 06 专项 112 项）；显式 XLA 套件 46 / 46（含 Chapter 06 GQA + QK-Norm smoke 9 项，编译 prefill / decode / 生成 / 训练与 CPU eager 一致）。回归证据：HEAD 版旧 `MultiHeadAttention` 加载到隔离模块，与新实现相同 seed 下参数树 `==`、输出逐元素 `==`；真实 Chapter 03 / Chapter 04 checkpoint 文件经新代码加载后 `num_kv_heads=num_heads`、`use_qk_norm=false`，forward 与 cached generation 正常。GQA 语义：manual reference / 无物化分组实现 / repeat_kv 展开三路在 6 组 head 组合 × causal on/off 下逐元素一致。benchmark：KV cache 内存严格按 `num_kv_heads / num_heads` 缩减（8→4→1 heads 对应 1024→512→128 KiB），dynamic decode 吞吐 2097→2484→3131 tok/s，correctness 全部为 true。
- **没有完成及原因**：未加载真实 HF 权重、未实现 safetensors / bfloat16 读取（Chapter 07 范围）；未导入 HF tokenizer.json（Chapter 08 范围）；CUDA GPU / XLA GPU 上的 GQA benchmark 未单独执行，CPU 数据已足以验证 cache 布局与吞吐趋势，GPU 对照留给 Chapter 07 加载真实权重后一并做。
- **最重要的认知变化**：结构 parity 的关键不在"实现了同名组件"，而在钉死每一处排列约定——head→KV 路由、head 切分顺序、QK-Norm 的作用维度与位置都必须用 reference 测试固定，否则 Chapter 07 权重对不上时无法归因。写 HF 契约草案时发现的 RoPE 配对差异（HF rotate_half vs 本仓库 interleaved）证明了这类文档的价值：这是一个不写映射就一定会踩、且 logits 全错但 shape 全对的陷阱。
- **是否满足 Close 条件**：是，Chapter 06 于 2026-07-22 Closed。
- **带到下一 Chapter 的问题**：RoPE rotate_half 与 interleaved 的适配方案（新增模式 vs 权重 permutation）需要在 Chapter 07 一开始决定并用逐层 fixture 验证；safetensors 读取、bf16→f32 转换与 HF 参数名映射的实现顺序；logits 对齐的容差策略（以 HF float32 参考为基准）；`max_seq_len` 相对 `max_position_embeddings` 的取值策略。
