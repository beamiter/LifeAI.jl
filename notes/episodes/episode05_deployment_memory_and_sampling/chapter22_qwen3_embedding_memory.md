# Chapter 22 — Qwen3-Embedding-0.6B 与最小语义记忆
...
- Chapter 22 专项、默认完整测试以及可用硬件上的真实权重验证全部通过；
...
- 本 Chapter 不实现 reranker、持久向量数据库、ANN index 或 XLA resident
...
- 2026-07-31：Chapter 22 Open。官方 revision 冻结为
...
- `examples/qwen3_embedding_memory.jl` 实跑仓库四份 notes；默认 query
  的首项是 Chapter 22（score `0.5927`），随后是 Chapter 17 / Chapter 21。
...
- Chapter 22 离线专项 `93 / 93`，加真实 CPU 权重专项 `103 / 103`。
...
- **没有完成及原因**：reranker、ANN、持久化、增量更新、XLA resident
  service 与 agent memory policy 均不在本 Chapter 范围。
...
- **带到下一 Chapter 的问题**：优先进入 Qwen3-VL 的视觉输入与最小图文
