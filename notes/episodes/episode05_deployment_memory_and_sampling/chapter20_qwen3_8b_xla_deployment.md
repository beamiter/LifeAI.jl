# Chapter 20 — Qwen3-8B XLA 单驻留 4K 日常部署

> 状态：Closed（2026-07-30）

> 依赖基线：[`Chapter 19 — Qwen3-8B / RTX 4090 D 日常本地部署`](../../episodes/episode05_deployment_memory_and_sampling/chapter19_qwen3_8b_4090d_deployment.md)
> 已把同一冻结模型落成 4K CUDA eager 日常入口，但 3,584-token prefill
> 为 46.85 s、整窗 decode 为 10.25 tok/s。Chapter 20 不改变模型、revision
...
prefill/decode 把 Chapter 19 的 3,584+512 日常窗口提速，同时守住 CUDA
...
等价于本 Chapter 已关闭的 4090D 显存 profile。
...
XLA benchmark 不用自身输出当 reference。先由 Chapter 19 CUDA eager session
导出 schema 2 oracle：
...
相对 Chapter 19 同一模型、同一 4K 窗口的 CUDA eager 最终值，XLA 的
...
- [x] Chapter 20 专项、独立 XLA smoke 与默认回归通过：
...
- Chapter 19 已定位 8B packed-XLA 的阻塞点：ordinary tree 与 packed
...
JSON；allocator 的内部 estimate 不再能单独关闭本 Chapter。
...
Chapter 20 消除了 8B XLA 的关键容量问题：不是再压低模型精度，而是让
safetensors loader 直接产出 kernel 所需的唯一 compact tree，并只上传
一次。Qwen3-8B BF16 因此在同一张 RTX 4090 D 上保留 Chapter 19 的完整
