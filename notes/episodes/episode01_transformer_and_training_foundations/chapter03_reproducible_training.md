# Chapter 03 — Reproducible Training and Evaluation
...
Chapter 02 已经跑通训练、生成和 KV Cache。Chapter 03 不继续堆叠模型结构，而是先补齐实验基础设施：收束增量推理、保存完整训练状态、建立无泄漏的 validation 与 perplexity，并增加梯度裁剪。

## 对初步路线的判断

整体方向合理，但不建议在同一个 Chapter 同时修改训练系统、Transformer 结构、Tokenizer 和训练语料。这样会一次改变过多变量，出现质量或性能变化时难以定位原因。

更合理的依赖顺序是：

```text
Chapter 03：可恢复、可评估、可比较
    ↓
Chapter 04：RMSNorm、SwiGLU、embedding/lm_head 权重共享
    ↓
Chapter 05：byte-level baseline、BPE 与中文数据管线
    ↓
Chapter 06：GQA、较真实的小型中文语料训练与综合 benchmark
```
...
每个组件都应与 Chapter 03 建立的固定 baseline 做参数量、loss、速度和兼容性比较。不要把三个变化只作为一个不可拆分的新模型版本。

### Chapter 05：Tokenizer 与数据管线
...
### Chapter 06：GQA 与较真实的小型中文训练

- GQA 会改变 K/V head 数和 cache 布局，应复用 Chapter 03 的 full / dynamic / static correctness 与 benchmark 框架。
...
结果为 654 / 654 通过，其中 Chapter 03 新增的可复现实验测试为 57 / 57。覆盖：
...
- **验证证据**：默认测试 654 / 654 通过，Chapter 03 专项 57 / 57；最新四后端 benchmark 均为 `ok` 且 correctness 为 `true`；checkpoint 测试验证 logits、参数、optimizer state、step 和恢复后 loss 的一致性。
- **没有完成及原因**：没有加入 RMSNorm、SwiGLU、权重共享、Tokenizer 或真实语料；这些是为保持变量可归因而主动留给后续 Chapter 的范围，不是 Chapter 03 缺项。默认测试命令未包含显式启用的 XLA 专项套件，其覆盖边界已单独记录。
...
- **是否满足 Close 条件**：是，Chapter 03 于 2026-07-18 Closed。
- **带到下一 Chapter 的问题**：如何在不破坏 legacy baseline、checkpoint 和 KV Cache / XLA 路径的前提下，独立引入并比较 RMSNorm、SwiGLU 与 embedding / LM head 权重共享。
