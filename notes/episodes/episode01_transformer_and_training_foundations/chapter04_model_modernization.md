# Chapter 04 — Modern GPT Building Blocks

> 状态：Closed
>
> 关闭记录：2026-07-19
>
> 依赖基线：[`Chapter 03 — Reproducible Training and Evaluation`](../../episodes/episode01_transformer_and_training_foundations/chapter03_reproducible_training.md) 已 Closed。

## 已确认的执行边界

2026-07-18 审阅通过，按以下边界执行：

1. **范围**：本 Chapter 只做 RMSNorm、SwiGLU、embedding / LM head 权重共享及其兼容性与对照实验；不提前加入 BPE、GQA、真实语料或 agent 能力。
2. **兼容策略**：默认构造仍是 LayerNorm + GELU + untied head；Chapter 03 的模型配置和 checkpoint 必须可加载。
...
Chapter 03 已经解决实验“能否复现和比较”的问题。Chapter 04 使用该基线做受控结构实验：每次只改变一个组件，先证明公式、梯度和接口正确，再观察参数量、训练行为和推理性能变化，不因 tiny corpus 上的一次 loss 波动宣称模型质量提升。
...
4. modern 组合可以完成 train → validate → save → load → resume → generate，并能迁移加载 Chapter 03 checkpoint。
...
- 三个新开关的默认值必须重建 Chapter 03 架构，避免“升级依赖即改变 baseline”。
...
| --- | --- | --- | --- | --- |
| `baseline` | LayerNorm | GELU | untied | Chapter 03 固定参考 |
...
只有以下条件全部满足后才能关闭 Chapter 04：

- 默认 `GPTModel(...)` 仍构建 LayerNorm + GELU + untied head，Chapter 03 的现有测试与 legacy fixture 全部通过。
...
- modern 组合完成 train → validate → save → load → resume → generate；Chapter 03 checkpoint 通过 v1 → v2 迁移后 logits 一致。
...
- 默认测试 765 / 765 通过，其中 Chapter 04 专项 111 / 111。
- 显式 Reactant/XLA 专项 30 / 30 通过；加上默认套件共 795 项。
- Chapter 04 专项覆盖 RMSNorm / SwiGLU reference formula、异常输入、有限梯度、共享梯度合流、精确参数量差、legacy fixture、五配置 train-step/cache matrix、modern checkpoint round-trip / resume / generation 和 checkpoint v1 → v2 迁移。
...
结果没有支持“把三个现代组件全部打开就必然改善 validation”的简单结论：短训练中 `tied_only` 最好，`modern` 差于 baseline，`rmsnorm_only` 最差。tied 配置还因复用当前 embedding 初始化而具有更高的初始 PPL。它们是后续初始化与训练策略实验的线索，不是修改本 Chapter 固定基线来追求更好数字的理由。
...
- **验证证据**：默认测试 765 / 765、Chapter 04 专项 111 / 111、显式 XLA 专项 30 / 30；五配置全部 cache-correct；baseline/modern 的 CPU、CUDA GPU、XLA CPU、XLA GPU 八组结果全部 `ok` 且 correctness 为 `true`。
- **没有完成及原因**：没有加入 BPE、GQA、真实语料、低精度专项或重新调参；前三项属于已确认非目标，低精度和调参需要独立变量与更真实训练规模，不应为改善本 Chapter tiny benchmark 数字而临时扩展范围。
...
- **是否满足 Close 条件**：是，Chapter 04 于 2026-07-19 Closed。
- **带到下一 Chapter 的问题**：进入 byte-level / BPE 与中文数据管线前，是否应同时冻结一套适合 tied embedding 的零中心初始化基线；Tokenizer 的 normalization、special token、版本持久化和无泄漏数据边界如何定义。
