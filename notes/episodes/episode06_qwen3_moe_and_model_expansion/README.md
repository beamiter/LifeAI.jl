...
- **形成的能力闭环**：Chapter 24 已完成 config/checkpoint/parity/sparse dispatch，Chapter 25 形成 RTX 4090 D offload session，Chapter 26 加入 byte-budgeted device LRU，Chapter 27 按模型逐层访问拓扑解决全局 LRU scan thrashing，Chapter 28 再消除命中路径的 active tensor materialization 并配置 GC cadence；Episode 仍为后续模型扩展保持 Open。
...
- **失败、偏差与未解决问题**：BF16 跨框架路由边界仍有差异；40K full-window/长序列质量尚未实跑；scattered 当前只支持 scalar CUDA BF16 cache，grouped WMMA、pointer/workspace reuse 与同步 miss path 尚未优化。完全关闭 forced GC 在 100 次短请求中损失 `0.9375 GiB` allocator free 并出现尾延迟，因此没有作为推荐配置。
- **重要架构决策**：correctness oracle、streamer 与 production dispatch 分层；cache 默认关闭且默认策略保持 global 兼容，真实 sequential-scan workload 显式使用 layer-balanced；scattered 与放宽 GC 均保持 opt-in，预算决策同时考虑 I/O、latency 与 GPU headroom。
- **最重要的认知变化**：BF16 支持和 cache hit 都必须落到具体生命周期与数据移动；只有 dtype/kernel/cache 状态正确，不能替代对 D2D copy、临时分配和 GC 的真实测量。
- **进入下一章的问题**：如何跨层/请求复用 pointer tables 与 route workspace，在长自然文本生成 trace 上验证稳定性，再用异步预取和 pinned-memory 双缓冲隐藏剩余 miss I/O。
