...
- **工程与测试**：测试按 Chapter 24 架构/parity、Chapter 25 offload session、Chapter 26 device LRU、Chapter 27 scan-resistant 策略/自然文本预算曲线、Chapter 28 scattered dispatch/GC、Chapter 29 pointer/workspace reuse、Chapter 30 bounded miss pipeline 与 Chapter 31 storage-aware worker sweep 分层。
...
- **进入下一章的问题**：如何合并同 shard 的小 tensor reads、复用 host decode/transpose buffers，减少 open/seek/syscall 与 allocator 压力；这可能比继续扩大 worker 数更可控。
