...
- **失败、偏差与未解决问题**：BF16 跨框架路由边界仍有差异；40K full-window/长序列质量尚未实跑；scattered 当前只支持 scalar CUDA BF16 cache，grouped WMMA 尚未接入。完全关闭 forced GC 的 500 次请求没有 live leak，但 allocator pool 仍扩张 `885.9 MB` 且 max `758 ms`。pinned async 比 overlapped pageable 慢 `1.07%`，layer-ahead prefetch 又受 router 数据依赖约束，因此两者都没有被包装成收益结论。
...
- **最重要的认知变化**：BF16 支持和 cache hit 都必须落到具体生命周期与数据移动；并行 read 的收益也不能自动归功于 pinned H2D。本机 `1.969×` 来自 bounded current-layer reads，而 pinned 版本略慢。
- **进入下一章的问题**：自然文本 layer-balanced trace 的最佳 worker 数是多少，冷盘与 warm page cache 是否同方向；如何进一步复用 host buffers，而不跨越 router 依赖或破坏容量门禁。
