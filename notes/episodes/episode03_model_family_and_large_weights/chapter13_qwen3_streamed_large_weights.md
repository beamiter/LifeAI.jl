# Chapter 13 — Qwen3 Streamed Loading and 8B/14B/32B Real-Weight Parity
...
> 关闭记录：2026-07-25
>
> 依赖基线：[`Chapter 12 — Qwen3 Dense Family Real-Weight Parity`](../../episodes/episode03_model_family_and_large_weights/chapter12_qwen3_dense_real_weights.md) 已 Closed，保持历史内容不变。
>
> 近期主线：Chapter 12 把真实权重 parity 扩展到 30 GiB RAM 内可全量加载的
...
三个尺寸全部 untied LM head——这是 Chapter 11 只有缩小模型证据、真实权重
...
2. **Python reference 侧**：Transformers Float32 计算下 32B 同样放不进
   ...
   生成与 Chapter 07/12 完全同口径的逐层 Float32 reference。
...
- 下载：8B（5 分片，≈ 16.4 GB）、14B（8 分片，≈ 29.5 GB）、32B
  （17 分片，≈ 65.1 GB），共 ≈ 111 GB，revision 沿用 Chapter 11 冻结值。
...
- Chapter 13 测试进入默认套件且不联网；默认全套与既有专项无回归。
...
- Chapter 12 保持 Closed；Chapter 13 承接最后三个 dense 尺寸的真实权重验证。
...
- 111 GB 三尺寸资产下载完成；全部文件 SHA256 已计算，三个
  `config.json` 与 Chapter 11 冻结值一致，tokenizer 三件套与 0.6B 字节
  相同。
...
- 把 Chapter 12 两个全量加载 integration（4B 峰值 ≈ 16 GiB）与 Chapter 13
  三个流式 integration 放进**同一个** `Pkg.test` 进程时，进程被内核
  OOM KILL——Julia 堆在多个大 testset 间累积增长，虽然单项峰值都远低
  于 30 GiB，叠加后仍越界。Chapter 13 单独进程 `545 / 545` 通过。
...
- 分进程协议下三轮全绿：week12 opt-in 全套通过（Chapter 12 专项
  `209 / 209`）；week13 opt-in 全套通过（Chapter 13 专项 `689 / 689` =
  离线 283 + 三尺寸 integration 406）；默认离线全套 `4652 / 4652`
  （Chapter 13 离线专项 `283 / 283`）。
- 本周 src 改动限于：新增 `src/io/hf_streaming.jl`、从
  ...
  不变，Chapter 07—12 离线测试逐项复核通过）；既有 Reactant/XLA 专项
  路径未改动。
