# Chapter 11 — Qwen3 Dense Family Completion
...
> 关闭记录：2026-07-23
>
> 依赖基线：[`Chapter 10 — GPT-2 Architecture, HuggingFace Weights and Text Parity`](../../episodes/episode03_model_family_and_large_weights/chapter10_gpt2_hf_parity.md) 已 Closed，保持历史内容不变。
>
> 近期主线：把 Chapter 06—09 在 Qwen3-0.6B 上建立的结构、权重和生成闭环扩展为显式的 Qwen3 dense family contract，同时严格区分“结构/loader 已覆盖”和“真实大权重已实跑”。
...
`rope_scaling` fail closed，因此本 Chapter 不把 128K 扩展写成已支持。
...
- QK-Norm 参数计数修复不影响 Chapter 06—10 的 logits、checkpoint 或默认测试。
...
- Chapter 10 保持 Closed；Chapter 11 单独承接 Qwen3 dense family 补全。
...
- **验证证据**：六份 immutable revision/config checksum fixture、六套完整
  ...
  全套 `4284 / 4284` 和 Chapter 11 XLA smoke `4 / 4`。
