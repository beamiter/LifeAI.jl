...
非法动作、终止状态、任务成功和跨进程 replay 变成严格可测的契约，再逐步扩展到事件记忆、故障语义
和真实或外部 simulator adapter。
...
- [ ] 环境事件按显式 policy 写入 Chapter 39 memory journal，并在后续 episode 中检索使用。
- [ ] 为超时、执行失败、急停和幂等 action 定义不依赖 GridWorld 的 adapter/safety 契约。

Chapter 40 已建立最小离散环境闭环，但后两项仍未完成，因此 Episode 08 保持 Open。
