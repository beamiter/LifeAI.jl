# Week 23 — Qwen3 XLA 设备端采样

> 状态：Closed（2026-08-01）

## Open：核心问题

Week 16 已经把 greedy 的 argmax 放进 executable，宿主每 token 只取回一个
...
2. 该策略在普通 Array 上与 Week 09 已验证的宿主策略
...
- 本 Week 不做设备端 RNG（counter-based）、不做 batch > 1 采样、
  不改 HTTP 服务的默认策略。
...
- 2026-08-01：Week 23 Open。基线默认测试 `5,582 / 5,582` 通过。
...
- 随机性仍然留在宿主：每步宿主只送 4 字节 uniform、取回一个整数，
  ...
  同一串 uniform 对拍两条路径。设备端 RNG 不在本 Week 范围。
...
- 真实权重验收（本机 RTX 5080 16 GiB、driver `595.71.05`、
  ...
  **38 / 38 token 完全一致，无分歧步**。
  ...
  prefill `0.0061` vs `0.0444` s。设备采样已经贴近 Week 16 冻结的
...
- 结论修正了一个直觉：`:sample` 慢十倍不是采样算法贵，而是每 token 传回
  ...
  几乎免费。
- Week 23 离线专项 `81 / 81`，加 Reactant CPU 编译对拍 `91 / 91`；
  默认完整测试 `5,663 / 5,663`（Week 22 基线为 `5,582`）。
...
- **没有完成及原因**：设备端 RNG、batch > 1 采样、把 `:device_sample`
  设为 HTTP 服务默认策略、8B 上的同类验收都不在本 Week 范围（8B BF16
...
- **带到下一 Week 的问题**：`:device_sample` 是否直接成为 XLA 常驻服务的
