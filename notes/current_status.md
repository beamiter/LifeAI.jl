...
2026-08-12 最新复核默认套件，共 `6,449 / 6,449` 项测试通过；Episode 06
为 `786 / 786`，其中 Chapter 24/25/26/27/28/29/30/31 分别为
`285 / 41 / 54 / 65 / 73 / 70 / 86 / 112`。该计数包含官方 30B-A3B immutable 资产契约、
...
- BF16/量化训练、FP8、完整 GPTQ/AWQ/Hessian/block reconstruction、
  ...
  （INT4 约 16.4 GiB）仍出界。Chapter 24–31 已完成 CPU Float32 correctness、
  ...
  但 full-window 40K prefill/长生成、grouped scattered、shard-coalesced reads、
  host buffer reuse 和 layer-ahead prefetch 尚未完成。pinned async 已可用，但
  本机没有胜过 pageable；cold/revisit worker sweep 已完成到 8 workers。
...
| 学习记录 | Chapter 01—31 已 Closed | 继续以论文/官方 reference、数值 parity、性能原始记录为近期节奏 |
