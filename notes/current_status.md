...
[`Week 08 — HuggingFace Qwen3 Tokenizer and Text-to-Text Parity`](week08_hf_tokenizer_text_parity.md) 已于 2026-07-22 Open，是「复现 Qwen3 → HF 权重加载 → 推理验证」三阶段计划的最后一步：严格导入 Qwen3 tokenizer 的 byte-unicode、regex、BPE 与 added/special token 语义，完成基础 chat template，以及 Qwen3-0.6B 的 text→ids→greedy generation→text 端到端对齐。Week 07 的真实权重与 logits parity 已完成。
...
- HF `tokenizer.json` 导入、GPT-2 byte↔unicode 映射、regex pre-tokenization、基础 Qwen3 chat template 与 text→text parity（Week 08 当前阶段）。
...
- 导入 HF `tokenizer.json`（byte-level BPE、byte↔unicode 映射、regex pre-tokenization、special tokens），完成基础 chat template 与 text→text 端到端一致性验证。（Week 08 已于 2026-07-22 Open）
...
| 主线 | 当前状态 | 下一关键缺口 |
| --- | --- | --- |
| 模型基本组件 | Qwen3 dense 同构结构；HF config / safetensors 权重加载；Qwen3-0.6B logits parity；三类版本化 Tokenizer 与中文数据管线 | Week 08 正在导入 HF tokenizer 并验证 text→text parity |
...
| 学习记录 | Week 01—07 已 Closed；Week 08 已 Open | 完成 HF tokenizer、基础 chat template 与 Qwen3 text→text parity 后 Close Week 08 |
