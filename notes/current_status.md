...
[`Week 07 — HuggingFace Weight Loading and Qwen3 Logits Parity`](week07_hf_weight_loading.md) 已于 2026-07-22 Open，是「复现 Qwen3 → HF 权重加载 → 推理验证」三阶段计划的第二步：实现 safetensors / bfloat16 权重加载、HF `config.json` 解析、RoPE rotate_half 适配与 Qwen3-0.6B 逐层 hidden states / logits / KV-cache 对齐。Week 06 的结构 parity 已完成；Week 08 将继续导入 HF tokenizer，完成 text→text 端到端验证。
...
| 学习记录 | Week 01—06 已 Closed；Week 07 已 Open | 完成 safetensors 权重加载与 Qwen3-0.6B logits 对齐后 Close Week 07 |
