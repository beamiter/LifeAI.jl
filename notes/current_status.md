...
项目已经形成一个可训练、可生成、可保存恢复、可评估比较，支持现代组件、KV Cache / XLA 路径，并具备版本化 Tokenizer 与文档级无泄漏中文数据管线的最小 decoder-only GPT；Qwen3-0.6B 的 text→ids、逐层 logits、full/dynamic/static greedy generation 已与 HF reference 对齐，当前正在补齐官方 sampling fidelity、长位置 correctness 与真实推理性能证据。

## 当前活动阶段

[`Week 09 — Qwen3 Sampling Fidelity and Real Inference Performance`](week09_qwen3_sampling_performance.md) 已于 2026-07-22 Open。近期核心目标调整为持续复现经典/SOTA 模型与论文；本 Week 在 Week 06—08 的 Qwen3-0.6B greedy 闭环之上，继续完成官方 temperature/top-k/top-p 分布、固定随机流 sampled generation、长位置 RoPE 与真实 cache 性能验证。
...
2026-07-22 复核默认套件，共 4135 项测试通过；其中 Week 05 专项 3094 项、Week 06 专项 112 项、Week 07 离线专项 54 项、Week 08 离线专项 61 项、Week 09 离线专项 46 项。Week 09 变更前显式设置 `LIFEAI_TEST_XLA=true` 时，默认套件与 Reactant/XLA 套件 52 / 52 通过；Week 09 尚未新增 XLA sampling kernel，本轮未重复执行该耗时专项。
...
Week 09 的官方 sampling reference 使用 Transformers 4.51.0、16 个固定 uniforms 和同一 Float32 compute reference：sampled integration 86 / 86 通过，raw/filtered/probability global max-abs 分别为 `6.67572e-5`、`3.05176e-5`、`5.90086e-6`。Qwen3-0.6B CPU benchmark 在 Intel Core Ultra 7 270K Plus 上完成；16/64/256-token prompt 的 dynamic cache decode 分别为 14.28/14.77/10.95 tok/s，256-token 时相对 full recompute 加速 10.33×；完整 raw samples 见 `benchmark_results/week09/`。
...
### Milestone B''：深化 Qwen3 真实生成与框架性能（进行中）

- 复现官方 temperature/top-k/top-p sampling，比较候选分布并用固定 uniform 流跨框架重放。（Week 09）
- 验证长位置 RoPE 边界和多 prompt length 的 full/dynamic/static cache correctness。（Week 09）
- 建立 Qwen3-0.6B CPU 必做、CUDA/XLA 据实记录的 load/prefill/decode/RSS/cache benchmark。（Week 09）

完成标准：官方 sampling 的候选 ids、filtered logits/probabilities 和固定 uniform token 与 Transformers 对齐；真实模型性能结论有原始实验条件和可重复命令，不把 BF16 storage 误写为 native BF16 compute。
...
| 主线 | 当前状态 | 下一关键缺口 |
| --- | --- | --- |
| 模型基本组件 | Qwen3 dense 同构结构；HF config / safetensors / tokenizer；Qwen3-0.6B greedy text→text parity；四类版本化 Tokenizer 与中文数据管线 | Week 09 补官方 sampling 与长位置验证；之后选择 native BF16、MoE 或下一经典模型 |
| 高效训练与推理 | modern / GQA / rotate_half 已兼容 Zygote / XLA 与两类 KV Cache；Qwen3-0.6B decode 已真实验证 | Week 09 建真实长度/per-cache 性能基线，再决定低精度与硬件优化 |
...
| 学习记录 | Week 01—08 已 Closed；Week 09 sampling/performance 已 Open | 以论文/官方 reference、数值 parity、性能原始记录为近期节奏 |
